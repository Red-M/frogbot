# -*- coding: utf-8 -*-
# cobalt wiki scrapper made by Red_M on esper.net or Red-M on GitHub.com
# direct clone of the minecraft wiki scrapper but for the cobalt wiki
...
def dbresponse(query,url,db_global):
    (desc, url, timemon, query) = db_global.execute("select desc, url, time, query from cobaltwiki where query=(?)",(query.lower(),)).fetchone()
    return("(Cached) "+desc)
...
def apiget(inp,url,requesturl,timemon,db_global):
    ...
    if not pageid=="-1":
        ...
        while url.startswith("#REDIRECT "):
            url2 = http.get_json("http://www.cobaltwiki.com/api.php?format=json&action=query&titles="+url.replace("#REDIRECT ","").replace(" ","%20")+"&prop=revisions&rvprop=content")["query"]
            url = dataget(url2)
        db_global.execute("insert or replace into cobaltwiki(desc, url, time, query) values (?, ?, ?, ?)",(url,requesturl,timemon,inp.lower()))
        db_global.commit()
        return(url)
    else:
        return(inp+" not found in the Cobalt wiki.")
        
@hook.command
def cobaltwiki(inp, db_global=None,input=None):
    ",cobaltwiki <query>/<item id> -- Returns Cobalt wiki search result for <query>/<item id>."
    db_global.execute("create table if not exists cobaltwiki(desc, url, time, query)")
    db_global.commit()
    if ((inp=="clear cache") and (perm.isowner(input))):
        db_global.execute("DELETE FROM main.`cobaltwiki`")
        db_global.commit()
        return("cache cleared.")
    timemon = time.strftime("%m", time.gmtime())
    if len(inp)>0:
        requesturl = "http://www.cobaltwiki.com/api.php?format=json&action=query&titles="+inp.replace(" ","%20")+"&prop=revisions&rvprop=content"
        dburl = db_global.execute("select query from cobaltwiki where query=(?)",(inp.lower(),)).fetchone()
        dbtime = db_global.execute("select time from cobaltwiki where query=(?)",(inp.lower(),)).fetchone()
