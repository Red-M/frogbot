...
def apiget(inp,url,requesturl,timemon,db_global,wiki,wikiname,wikiurl,wikilen):
    ...
    if not pageid=="-1":
        ...
        while url.startswith("#REDIRECT "):
            url2 = http.get_json("http://www."+wiki+"wiki.com/api.php?format=json&action=query&titles="+url.replace("#REDIRECT ","").replace(" ","%20")+"&prop=revisions&rvprop=content")["query"]
...
def cobaltwiki(inp, db_global=None,input=None):
    ",cobaltwiki <query> -- Returns Cobalt wiki search result for <query>."
    wikiname = "cobalt"
    wiki = "cobalt"
    wikilen = 1
    wikiurl = "http://www."+wiki+"wiki.com/"
    wikiwiki = "http://www."+wiki+"wiki.com/"
