...
from util import hook, perm, http, mcrcon, webserver
    ...
    def handle(self):
        ...
        if data=="bot term. shutdown. NOW":
            ...
            try:
                ...
                print("We have an error from the web backend of: "+str(msg))
...
def web(inp,input=None,bot=None):
    ...
    while True:
        ...
        for xcon in bot.conns:
            if bot.conns[xcon].conf['bouncer-server']==True:
                chanxcon.append([])
...
def client(ip, port, message):
    ...
    try:
        ...
        data = "("+str(ip)+":"+str(port)+") Error: '"+str(msg)+"'."
    finally:
        sock.close()
        return data
