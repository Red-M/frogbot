...
def client(ip, port, message):
    ...
    try:
        message = struct.pack(str(len(message))+"s",message)
        sock.send(message)
        #time.sleep(2)
        print message
        (data,address) = sock.recvfrom(4096)
        enddata = str(data).replace("  ","").replace("\\"+"\\","\\")
...
def tcp(inp,bot=None,input=None):
    ...
    if perm.isowner(input):
        return str(client(str(check[0]),int(check[1]),
            str(check2.decode("utf8","replace")))).decode("utf8","replace")
    return "Nope.avi"
