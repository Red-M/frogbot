...
def apiget(inp,url,requesturl,timemon,db_global,wiki,wikiname,wikiurl,wikilen):
    ...
    if not pageid=="-1":
        ...
        while url.startswith("#REDIRECT "):
            url2 = http.get_json("http://www."+wiki+".wikia.com/api.php?format=json&action=query&titles="+url.replace("#REDIRECT ","").replace(" ","%20")+"&prop=revisions&rvprop=content")["query"]
